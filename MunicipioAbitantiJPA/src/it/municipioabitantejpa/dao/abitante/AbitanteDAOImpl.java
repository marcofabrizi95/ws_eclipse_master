...
public class AbitanteDAOImpl implements AbitanteDAO {
	...
	public List<Abitante> findAllByCognome(String cognome) throws Exception {
		return entityManager.createQuery("SELECT a.nome FROM Abitante a where a.cognome LIKE '"+cognome+"%'").getResultList();
	}
	@SuppressWarnings("unchecked")
	@Override
	public List<Abitante> findAllByCodiceComuneIniziaCon(String input) throws Exception {
		return entityManager.createQuery("SELECT m.abitanti FROM Abitante a join a.municipio m where  m.codice='"+input+"'").getResultList();
	}
