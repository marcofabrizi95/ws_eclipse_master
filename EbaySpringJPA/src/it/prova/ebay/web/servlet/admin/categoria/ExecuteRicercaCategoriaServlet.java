...
public class ExecuteRicercaCategoriaServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Long temp = -1L;
		try {
			temp = Long.parseLong(request.getParameter("prezzoInput"));
		} catch (Exception e) {
			// TODO: handle exception
		}
		Annuncio annuncio = AnnuncioDTO.buildAnnuncioInstance(
				new AnnuncioDTO (request.getParameter("testoAnnuncioInput"),temp));

		request.setAttribute("listaCategorieAttribute", categoriaService.findByExample(new Categoria(annuncio)));
