...
public class ExecuteEliminaCategoriaServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (Annuncio annuncio : categoriaDaEliminare.getAnnuncio()) {
			if (annuncioService.getEager(annuncio.getId()).getCategorie().size() <= 1) {
				request.setAttribute("messaggioDiErrore",
						"Per alcuni annunci questa è l'unica categoria associata. Non si può procedere all'eliminazione");
		...
		categoriaService.delete(categoriaDaEliminare);
